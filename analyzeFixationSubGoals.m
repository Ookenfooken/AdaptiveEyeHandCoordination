...
for j = 1:numParticipants % loop over subjects
    for i = 3:numBlocks % loop over dual blocks 
        ...
        currentVariable = [participant testID reachOnset ...
            ballApproach ballGrasp transport slotApproach slotEntry ...
            ballFixations ballOnsets ballOffsets ...
            slotFixation slotOnsets slotOffsets ];
...
ballFixations = [variables(:,1:2) variables(:,9) variables(:,3:8) variables(:,10:11)]; % paste relevant ball data
...
for tool = 3:4
    ...
    for i = 1:numParticipants
        ...
        currentPhases = currentParticipant(:,4:9) - mean(currentParticipant(:,4:9));
        ballFixations_adjusted = [ballFixations_adjusted; ...
            [currentParticipant(:,1:3) currentPhases currentParticipant(:,10:11)]];
    end
end

slotFixations = [variables(:,1:2) variables(:,12) variables(:,3:8) variables(:,13:14)]; % paste relevatn slot data
