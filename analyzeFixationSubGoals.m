    ...
    for i = 3:numBlocks % loop over dual blocks 
        ...
        for n = 1:stopTrial % loop over trials for current subject & block
            ...
            if ~isempty(currentResult(n).gaze.fixation.onsetsBall)
                ...
                ballOffsets(n) = currentResult(n).gaze.fixation.offsetsBall(1)+trialStart-startReach;
            ...
            if ~isempty(currentResult(n).gaze.fixation.onsetsSlot)
                ...
                if numel(currentResult(n).gaze.fixation.onsetsSlot) > 1
                    ...
                    if ~isempty(slotIndx)
                        ...
                        slotOffsets(n) = currentResult(n).gaze.fixation.offsetsSlot(slotIndx)+trialStart-startReach;
                    else
                        slotOnsets(n) = currentResult(n).gaze.fixation.onsetsSlot(1)+trialStart-startReach;
                        slotOffsets(n) = currentResult(n).gaze.fixation.offsetsSlot(1)+trialStart-startReach;
                    end
                else
                    slotOnsets(n) = currentResult(n).gaze.fixation.onsetsSlot+trialStart-startReach;
                    slotOffsets(n) = currentResult(n).gaze.fixation.offsetsSlot+trialStart-startReach;
                end
...
glmData_ball = [variables(:,1:2) variables(:,10) variables(:,3:9) variables(:,11:13)]; % paste relevant ball data
glmData_slot = [variables(:,1:2) variables(:,14) variables(:,3:9) variables(:,15:17)]; % paste relevatn slot data
%%
cd(fullfile(pwd,'R\'))
