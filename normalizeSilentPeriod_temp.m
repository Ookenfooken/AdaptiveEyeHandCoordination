...
for i = 3:numBlocks % plot per block aka experimental condition
    ...
    averagedTimeLC = NaN(numSubjects, averageLength);
    ...
    missFixationType = NaN(numSubjects*2, averageLength+1);
    ballFixationType = NaN(numSubjects*2, averageLength+1);
    slotFixationType = NaN(numSubjects*2, averageLength+1);
    ...
    for j = 1:numSubjects
        ...
        timeLetterChange = NaN(numTrials, averageLength);
        silentPreReach = NaN(numTrials, averageLength);
        missedChanges = NaN(numTrials, averageLength+1);
        ...
        for n = 1:stopTrial
            ...
            normalizedData = normalizeSilentPeriod(phaseDurationNorm, trialInfo, rawGaze, dualInfo, dualPrevious, i, radius);
            silentPeriod(n,:) = [fixationPattern normalizedData.silentPeriod];
            timeLetterChange(n,:) = normalizedData.timeLC;
            silentPreReach(n,:) = normalizedData.silentPreReach;
        ...
        averagedTimeLC(j,:) = nanmean(timeLetterChange);
    ...
    plot(nanmean(averagedEarlyReaches), 'Color', lightBlue, 'LineWidth', 2)
